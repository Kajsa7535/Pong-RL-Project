...
class ReplayMemory:
    ...
    def push(self, obs, action, next_obs, reward, terminated):
        ...
        self.memory[self.position] = (obs, action, next_obs, reward,terminated.to(device))
...
class DQN(nn.Module):
    ...
    def act(self, observation, exploit=False):
        ...
        if exploit or random_number > epsilon:
            with torch.no_grad():
                actions =  torch.argmax(self.forward(observation), dim=1)
                
        else:
            actions =  torch.randint(0, self.n_actions, (observation.size(0), ), device=device)
...
def optimize(dqn, target_dqn, memory, optimizer):
    ...
    states, actions, next_states, rewards, terminated = transitions
   ...
   # non_final_mask = torch.tensor([s is not None for s in next_states], device=device, dtype=torch.bool)
    ...
    #next_state_with_zeros = tuple(tensor.to(device) if tensor is not None else torch.zeros(1, 4) for tensor in next_states)
    ...
    expected_q_values =  rewards + (1-terminated) * next_q_values * dqn.gamma
